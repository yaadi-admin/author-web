...
export function MessageItem({ className, message, selectedChannel, setSelectedChannel }: MessageItemProps) {
  const hoverRef = useRef(null);
  const router = useRouter();
  const pageID = (window.location.pathname).split("/").pop();
  const isHover = useHover(hoverRef);
  const currentUser = currentSession() as any;
  const { messages: messageData } = chats() as any;
  const dataAtomMessage = atomWithReset<MessageType[]>(messageData);
  const [data, setData] = useAtom(dataAtomMessage);
  ...
  const handleItemChange = (itemId: string) => {
    ...
    // toast.success(`${itemId}`, { position: "bottom-center" });
...
export default function ChatList({ className, selectedChannel, setSelectedChannel }: InboxListProps) {
  const currentUser = currentSession() as any;
  const { messages: messageData } = chats() as any;
  ...
  // useEffect(() => {
  //   const updatedItems = messageData.filter(
  //     (item: { status: string; }) => item.status === supportStatuses.Open
  //   );
  //   setData(updatedItems);
  ...
  // const handleClosed = () => {
  //   const updatedItems = messageData.filter(
  //     (item: { status: string; }) => item.status === supportStatuses.Closed
  //   );
  //   setData(updatedItems);
        ...
        <div className="overflow-hidden rounded-lg border border-muted">
          <SimpleBar className="max-h-[calc(100dvh-356px)] md:max-h-[calc(100dvh-311px)] lg:max-h-[calc(100dvh-240px)] xl:max-h-[calc(100dvh-230px)] 2xl:max-h-[calc(100dvh-240px)] 3xl:max-h-[calc(100dvh-270px)]">
            {messageData?.map((message: any) => (
              <MessageItem selectedChannel={selectedChannel} setSelectedChannel={setSelectedChannel} key={message.id} message={message} />
            ))}
          </SimpleBar>
        </div>
      </div>
