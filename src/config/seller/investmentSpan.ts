...
const COLLECTION = 'investment_span';

export function investmentSpan() {
  const currentUser = currentSession() as any;

  const [investmentSpan, setInvestmentSpan] = useState([]) as any;
  const [progress, setProgress] = useState(0);
  const { investmentSpan: investment_collection } = investmentCollection() as any;

  const currId = currentUser?.id;

  useEffect(() => {
    if (investment_collection && currId) {
  ...
  const fetchSellerSpanData = async () => {
    ...
    try {
      ...
      investment_collection.forEach((template: any) => {
  ...
  const updateIntake = async (data: any) => {
    ...
    const docRef = doc(firebase.firestore, 'invest_intakes', currIntake?.id);
    await updateDoc(docRef, data);
  };
