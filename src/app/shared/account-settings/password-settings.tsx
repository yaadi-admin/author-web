...
export default function PasswordSettingsView({
  ...
  const [alternateEmail, setAlternateEmail] = useState(currentUser?.alternateEmail);
  ...
  const handleReset = async () => {
    await sendPasswordResetEmail(firebase.auth, currentUser?.email)
      .then(() => {
        toast.success('Password reset email sent!', { position: "bottom-center" });
        setRequest(true);
      })
      .catch((error) => {
          ...
          return (
            <>
              ...
              <div className="mx-auto w-full max-w-screen-2xl">
                ...
                <HorizontalFormBlockWrapper
                  title={!request ? "Request a password update, to have a password reset link sent to your email" : `Password request sent to ${email}`}
                  titleClassName="text-base font-medium"
                >
                  <div className="mt-6 flex w-auto items-center justify-end gap-3">
                    {/* {password && confirmPassword && <Button type="button" variant="outline">
                    Cancel
                  </Button>} */}
                    {/* {password && confirmPassword &&  */}
                    <Button type="submit" variant="solid" onClick={handleReset} disabled={request} isLoading={isLoading}>
