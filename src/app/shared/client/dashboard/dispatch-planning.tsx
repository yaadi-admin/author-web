...
export default function DispatchPlanning({
  ...
  return (
    <WidgetCard
      ...
      description={
        <p>
          <span className="font-primary text-xl font-semibold text-gray-900 2xl:text-2xl">
            56,000
          </span>{' '}
          Truck Loads
        </p>
      }
      titleClassName="font-normal text-gray-500 text-sm sm:text-sm 2xl:text-base font-secondary"
        ...
        {data.map((item) => (
          <div key={item.name} className={cn('grid gap-6 text-center')}>
            <CircleProgressBar
              ...
              label={
                <Text className="font-primary text-sm font-bold text-gray-900 2xl:text-base dark:text-gray-700">
                  {item.value}
                </Text>
              }
