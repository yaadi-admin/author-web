...
export default function EventCalendarView() {
  ...
  const fetchEvents = async () => {
    try {
      const q = query(
        collection(firebase.firestore, "events"), where("userID", "==", currentUser?.id)
      );
      ...
      for (const event of dataFields) {
        const updatedEvent = {
          ...event,
          end: new Date(event.end.seconds * 1000 + event.end.nanoseconds / 1000000),
          start: new Date(event.start.seconds * 1000 + event.start.nanoseconds / 1000000),
          // start: new Date(Date.parse(event.start)),
          // end: new Date(Date.parse(event.end))
        };
        updatedDataFields.push(updatedEvent);
      }
      setEvents(updatedDataFields);
    } catch (error) {
      console.error('Error fetching data:', error);
      // Handle error (e.g., display error message, retry logic)
    }
