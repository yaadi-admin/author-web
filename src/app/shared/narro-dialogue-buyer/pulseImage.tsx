      ...
      {isUser ? (
        <div className="relative w-32 h-32 sm:w-40 sm:h-40 md:w-52 md:h-52 lg:w-64 lg:h-64 rounded-full">
          {currentUser?.profilePictureURL ? (
            <img
              ...
              alt=""
              className={`rounded-full shadow-2xl object-contain w-full h-full relative z-10 bg-white transition-transform ${isProcessingSpeech ? 'pulse-enlarge' : ''
                }`}
            />
          ) : (
            <span
              className={`rounded-full shadow-2xl flex items-center justify-center w-full h-full z-10 bg-secondary text-secondary-foreground ${isProcessingSpeech ? 'pulse-enlarge' : ''
                }`}
            >
        ...
        <div className="rounded-full w-32 h-32 sm:w-40 sm:h-40 md:w-52 md:h-52 lg:w-64 lg:h-64 relative z-10 flex items-center justify-center bg-green-200 font-regular">
          <img
            src={aigentImg}
            alt=""
            className={`rounded-full shadow-2xl object-contain w-full h-full relative z-10 bg-white transition-transform ${isRequestingStream ? 'bg-green-500 animate-pulse' : ''
              }`}
          />
        </div>
      )}
