...
export default function MultiStepSeller({ template, onNext }: any) {
  ...
  //     try {
  //       const q = query(collection(firebase.firestore, "intakes"), where("userID", "==", currentUser?.id));
  ...
  useEffect(() => {
    if (template) {
      const { section2 } = template;
      const tempArr = [];
      if (section2.isShown) {
        tempArr.push({ dialog: section2, CustomComponent: Dialog, id: 'dialog1' });
      }
      setCards(tempArr);
