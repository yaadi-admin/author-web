...
const scoreOptions = [
  {
    value: 'greaterThan60LessThan80',
    label: 'Score > 60 and < 80',
  },
  {
  ...
  return (
    <div className="flex w-full flex-col items-center gap-3 @[22rem]:flex-row @[35rem]:w-auto @[57rem]:-ms-4">
      ...
      <StatusField
        ...
        getOptionValue={(option: { value: any }) => option.value}
        displayValue={(selected: string) =>
          scoreOptions.find((option) => option.value === selected)?.label ?? ''
        }
        placement="bottom-start"
      /> */}
      {isFiltered ? (
...
function renderOptionDisplayValue(value: string) {
  switch (value.toLowerCase()) {
    ...
    case 'greaterthan60lessthan80':
      return (
        <div className="flex items-center">
          <Badge color="warning" renderAsDot />
          <Text className="ms-2 font-medium capitalize text-yellow-dark">
            Score &gt; 60 and &lt; 80
          </Text>
        </div>
      );
    case 'greaterthanorequal80':
      return (
        <div className="flex items-center">
          <Badge color="success" renderAsDot />
          <Text className="ms-2 font-medium capitalize text-green-dark">
