...
export default function Dialogues(props: DialoguesProps) {
  const { document, intakeSource } = props;
  ...
  React.useEffect(() => {
    async function getIntakes() {
      try {
        const q = query(collection(firebase.firestore, intakeSource), where("userID", "==", currentUser?.id));
  ...
  const onFinish = async (data: any) => {
    ...
    const docRef = doc(firebase.firestore, intakeSource, lastDoc.id);

    if (docRef) {
      await updateDoc(docRef, dialogueIntakeObj)
    }
    ...
    push(routes.realEstateSeller.congratulations)
    setLoading(false);
