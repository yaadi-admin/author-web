...
export default React.memo(function MessageBody(props: MessageBodyProps) {
  const { messages = [], promptStatus } = props;
  ...
  }, [messages, latestMessageRef, botThinking]);
  ...
  // console.log('user', isMessageSent, 'bot', botThinking)
  ...
  if (size > 0) {
    return (
      <div>
        {messages.filter((val, index) => {
          ...
          .map((message, index) => {
            ...
            return (
              <div
                id={message?.id}
                ref={index === size - 2 ? botThinking ? null : latestMessageRef : null}
                role={message?.role}
        ...
        {/* <div
          ref={isMessageSent ? latestMessageRef : null}
          role={'user'}
          style={isMessageSent ? { display: 'block' } : { display: 'none' }}
        >
          <div style={{
            marginTop: '5%',
            marginBottom: '5%',
