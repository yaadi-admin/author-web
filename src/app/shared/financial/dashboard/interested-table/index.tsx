...
import { getColumns } from '@/app/shared/financial/dashboard/interested-table/columns';
...
export default function InterestedTable({
  ...
  const fetchTransactionHistory = async () => {
    try {
      ...
      // Fetch listings for the current user using their email
      const listing = await fetchBySellerEmail(currentUser.email);
      if (!listing || !listing.id) {
        console.warn('No listing found for the current user.');
        setTransactionHistory([]);
        setPageSize(DEFAULT_PAGE_SIZE);
        setIsLoading(false);
        return;
      }
      const myListingId = listing.id;
      console.log('My listing ID:', myListingId);
      const querySnapshot = await getDocs(collection(firebase.firestore, 'offers'));
      let data: { id: string; [key: string]: any }[] = [];
      if (!querySnapshot.empty) {
        data = querySnapshot.docs
          .map((doc) => ({ id: doc.id, ...doc.data() }))
          .filter((item: any) => item.businessID === myListingId);
      }
      setTransactionHistory(data);
      setPageSize(Math.min(DEFAULT_PAGE_SIZE, data.length));
  ...
  return (
    <WidgetCard
      ...
      description={`
        Investors that have placed interest and that you have placed interest in.
      `}
      descriptionClassName="text-sm font-normal text-gray-500 pr-5 w-[500px] @[100rem]:pr-5"
      title="My Interested Investors"
      action={
