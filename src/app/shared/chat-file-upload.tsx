...
export const FileInput = ({
  ...
  const uploadStoredFIle = async (file: any) => {
  ...
  const storeFile = async (file: any) => {
    ...
    localStorage.setItem('fileDownloadURL', downloadURL);
    setFileDownloadURL(downloadURL);
    uploadStoredFIle(files[0]);
    setFileDescription('');
    setFileCategory('');
    setFileDownloadURL('');
  };
  ...
  return (
    <div className={className}>
      ...
      <Input
        label={"File Category (Financial etc.)"}
        type="text"
        placeholder="File category"
        className="rounded-md bg-gray-0 dark:bg-gray-50 flex-grow mr-2.5 [&>.ql-container_.ql-editor]:min-h-[100px] mt-4"
        value={fileCategory}
        onChange={(e) => setFileCategory(e.target.value)}
      />
      <Input
        label={"Write a short Description of this File"}
        type="text"
        placeholder='Write a short Description of this File'
        className="rounded-md bg-gray-0 dark:bg-gray-50 flex-grow mr-2.5 [&>.ql-container_.ql-editor]:min-h-[100px] mt-4"
