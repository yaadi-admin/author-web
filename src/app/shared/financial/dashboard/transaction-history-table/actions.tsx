...
import { Text, Checkbox, ActionIcon, Tooltip, Drawer, Button, Popover } from 'rizzui';
...
import { InvestorMatchSummary } from './profile';
...
export function Actions({ row, onDeleteItem }: ActionsProps) {
    ...
    // Check if the offer already exists for the investor
    useEffect(() => {
        const checkOffer = async () => {
            // Using row.id as the document ID
    ...
    const handleInterested = async (row: any) => {
        ...
        try {
            const docSnap = await getDoc(offerDocRef);
            // Prepare the full object including the new fields to append
            const offerData = {
                ...row, // include all fields from the full object
                interestDate: serverTimestamp(),
                cimRequest: 'approved',
                ddRequest: 'approved'
            };
            if (docSnap.exists()) {
                // Append new fields to existing document without removing old fields
                await updateDoc(offerDocRef, offerData);
            } else {
                await setDoc(offerDocRef, offerData);
            }
            // Optionally update button state after interest is expressed
            setIsInterestedDisabled(true);
            setOpenDrawer(true);
        } catch (error) {
    ...
    return (
        <>
            <div className="flex items-center justify-end gap-3 pe-3">
                    ...
                    {({ setOpen }) => (
                      <div className="w-56 pb-2 pt-1 text-left rtl:text-right">
                        ...
                        <Text className="mb-2 leading-relaxed text-gray-500">
                          Expressing interest in {row.investor.data.firstName}{' '}
                          will grant access to the business&apos;s files and reports. This action cannot be undone.
                        </Text>
                ...
                <Tooltip size="sm" content="Chat" placement="top" color="invert">
                    <ActionIcon
                        ...
                        onClick={() => handleOpenMessenger(row?.investor?.data?.user, hello)}
                    >
                ...
                <DeletePopover
                    title={`Remove ${row.investor.data.firstName} ${row.investor.data.lastName}?`}
                    description="Are you sure you want to remove this investor from the list?"
            ...
            <Drawer
                ...
                customSize="40%"
            >
                <InvestorMatchSummary
                    data={row}
                    chat={() => handleOpenMessenger(row?.investor?.data?.user, hello)}
                />
            </Drawer>
        </>
    );
