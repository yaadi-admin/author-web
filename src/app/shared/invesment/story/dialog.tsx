...
import { investmentSpan } from "@/config/seller/investmentSpan";
import { investmentCollection } from "@/config/ref/investmentCollections";
import { useCapitalMarket } from "@/config/seller/useCapitalMarket";
...
export default function Dialog(props: Dialog1Props) {
  const { investmentSpan } = investmentCollection();
    const { data, updateIntake, getIntake } = investmentSpan() as any;
  const { push } = useRouter();
  const configCardData = investmentSpan[0]
  const currentUser = currentSession() as any;      
    const { fetchBySellerEmail } = useCapitalMarket('pending');
        ...
        if (response) {
          ...
          eventSource.onmessage = async (event) => {
            ...
            if (eventData.event === ThreadEvents.THREAD_MESSAGE_COMPLETED) {
              ...
              push(routes.investment.congratulations)
              setLoading(false);
            }
          };
