...
const fetchData = async () => {
  ...
  return [
    {
      title: 'Investor Pitch Meetings',
      value: 3,
      total: 8,
      color: '#2B7F75',
    },
    {
      title: 'Broker Consultations',
      value: 3,
      total: 5,
      color: '#176B87',
    },
    {
      title: 'Advisory & Due Diligence Sessions',
      value: 1,
      total: 5,
      color: '#FFD66B',
    },
...
export default function CommunicationCenter({ className }: { className?: string }) {
  const [value, onChange] = useState<Value>(new Date());
  const [data, setData] = useState<{ title: string; value: number; total: number; color: string }[]>([]);
  ...
  return (
    ...
    >
      <div className="grow p-5 @lg:w-3/5 lg:p-7">
        <Title
          as="h3"
          className="pb-4 font-secondary text-base font-semibold text-gray-800 sm:text-lg"
        >
          Schedule
        </Title>
        <Calendar
          ...
          prev2Label={false}
          next2Label={false}
          className="!w-full !border-0 !bg-transparent !font-secondary !text-base"
        />
      </div>
