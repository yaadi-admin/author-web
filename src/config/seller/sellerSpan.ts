...
export function sellerSpan() {
                    ...
                    if (userCardIds.includes(template.id)) {
                        ...
                        cardsData.push({
                            ...
                            section1: {
                                ...currCard.section1,
                                ...template.section1,
                                isShown: template.section1.isShown,
                            },
                            section2: {
                                ...currCard.section2,
                                ...template.section2,
                                isShown: template.section2.isShown,
                            },
                            section3: {
                                ...currCard.section3,
                                ...template.section3,
                                isShown: template.section3.isShown,
                            },
                            section4: {
                                ...currCard.section4,
                                ...template.section4,
                                isShown: template.section4.isShown,
                            },
                            section5: {
                                ...currCard.section5,
                                ...template.section5,
                                isShown: template.section5.isShown,
                            }, // override the section 5
                            section6: {
                                ...currCard.section6,
                                ...template.section6,
                                isShown: template.section6.isShown,
                            },
                            isTemplate: false,
                        });
                    } else {
    ...
    const addSellerSpanSubCollection = async (card: any) => {
        const docRef = doc(firebase.firestore, "sellerspan", currentUser?.id, "cards", card.id);
    ...
    const updateSellerSpanSubCollection = async (id: string, data: any) => {
        const docRef = doc(firebase.firestore, "sellerspan", currentUser?.id, "cards", id);
        await updateDoc(docRef, data);
    };

    const updateProgress = async (card: {tag: string, data: string, formData: any}) => {
        ...
        const progressDocRef = doc(firebase.firestore, "sellerspan", currentUser?.id)
    ...
    const updateAdvisory = async (advisors: any[]) => {
        const nextAdvisors = advisors.filter((advisor: { name: string; }) => advisor.name.trim() !== '')
        const teamDocRef = doc(firebase.firestore, 'team', currentUser?.id);
        await setDoc(teamDocRef, {advisors: nextAdvisors});
        // const addAdvisorsToFirestore = async () => {
        //     try {

        //       const addAdvisorPromises = nextAdvisors.map(async (advisor) => {
        //         const reqData = {
        //             email: advisor.email,
        //             name: advisor.name,
        ...
        //             userID: currId,
        //         };
        //         await addDoc(progressCollection, reqData);
        //       });
          
        //       // Wait for all the advisor additions to complete
        //       await Promise.all(addAdvisorPromises);
        //       console.log('All advisors have been added successfully');
        //     } catch (error) {
        //       console.error('Error adding advisors to Firestore: ', error);
        //     }
        //   };
          
        // await addAdvisorsToFirestore()
    };


    const getProgress = async () => {
        if(currentUser.id) {
        const docRef = doc(firebase.firestore, "sellerspan", currentUser?.id)
        const response = await getDoc(docRef);
